...
async def update_incidencia(
    ...
    # ── Optimistic locking: detectar edicion simultanea ──────────────────────
    client_updated_at = data.pop("updated_at", None)
    if client_updated_at and client_updated_at != antes.get("updated_at"):
        conn.close()
        raise HTTPException(
            status_code=409,
            detail="Conflicto: otro usuario modifico esta incidencia mientras la editabas. Recarga e intentalo de nuevo."
        )
    # ─────────────────────────────────────────────────────────────────────────
