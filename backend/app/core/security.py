...
# Pre-hash con sha256 antes de bcrypt (evita limite de 72 bytes)
...
def _get_user_from_token(token: str) -> dict:
    ...
    conn = get_connection()
    user = conn.execute(
        "SELECT id, nombre, email, rol, activo FROM usuarios WHERE id=?", (user_id,)
    ).fetchone()
    conn.close()
...
async def get_current_user(
    ...
    if bearer_token:
        return _get_user_from_token(bearer_token)
